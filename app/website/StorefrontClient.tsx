...
function purchaseHref(product: WebsiteProduct) {
  if (!product.url) return product.merchantUrl || "#";
  const params = new URLSearchParams({ url: product.url, productId: product.id, source: "website" });
...
function imageHref(product: WebsiteProduct) {
  if (!product.image && !product.merchantUrl) return "";
  const params = new URLSearchParams({ productId: product.id });
  if (product.image) params.set("url", product.image);
  if (product.merchantUrl) params.set("fallback", product.merchantUrl);

...
const NAV = [["/", "⌂", "Tổng quan"], ["/ceo", "♛", "AI CEO"], ["/agents", "◈", "AI Agents"], ["/workflows", "⌁", "Workflows"], ["/history", "◷", "Lịch sử"]] as const;
...
export default function CommandOverview() {
  ...
  return <div className={styles.app}>
    ...
    <main className={styles.main}>
      <header className={styles.header}><div><span className={styles.eyebrow}><i /> AI CONTROL CENTER</span><h1>Trung tâm điều hành</h1><p>1 AI CEO điều phối · 20 AI thực thi độc lập · cô lập nhánh + channel.</p></div><div className={styles.headerLive}>{connected ? "● SYSTEM LIVE" : "● SYNCING"}</div></header>
      <section className={styles.hero}><div><span className={styles.eyebrow}>COMMAND CENTER · LIVE TELEMETRY</span><h2>AgentFlow đang vận hành.</h2><p>Primary live: SalesBot (Website) + Marketing (Facebook). Registry-only chưa bật runtime.</p></div><div className={styles.orbit} aria-hidden="true"><div>AF</div></div></section>
      ...
      <section className={styles.commandPanel} aria-label="Primary agent KPI"><div className={styles.commandHeading}><div><span className={styles.eyebrow}>REVENUE CAMPAIGN · KPI</span><h3>2 AI chủ lực · mục tiêu tháng</h3></div><span className={styles.headerLive}>30.000.000 ₫ TARGET</span></div>
        <div className={styles.kpiGrid}>
          {(kpiLoading ? ["SalesBot", "Marketing"] : kpis).map((item: KPI | string) => {
            const k = typeof item === "string" ? null : item;
            return <article className={styles.kpiCard} key={typeof item === "string" ? item : item.agentId}>
              <div className={styles.cardTop}><span>{typeof item === "string" ? item : item.name}</span><b className={k?.progress && k.progress > 0 ? styles.good : styles.warn}><i /> {k ? `${Math.round(k.progress)}%` : "SYNC"}</b></div>
              <div className={styles.kpiNumbers}><strong>{k ? money(k.actual) : "—"}</strong><span>/ {money(k?.target ?? 15_000_000)}</span></div>
              <div className={styles.progress}><span style={{ width: `${k?.progress ?? 0}%` }} /></div>
              <div className={styles.miniRow}><span>Còn lại <b>{k ? money(k.remaining) : "—"}</b></span><span>GD <b>{k?.transactions ?? "—"}</b></span></div>
            </article>;
          })}
        </div>
      </section>

      <section className={styles.commandPanel}><div className={styles.commandHeading}><div><span className={styles.eyebrow}>AI CEO · COMMAND BAR</span><h3>Ra lệnh / xem cockpit</h3></div><a href="/ceo">Mở AI CEO Cockpit →</a></div><form onSubmit={submit} className={styles.commandForm}><span>✦</span><input value={command} onChange={e => setCommand(e.target.value)} placeholder="Ví dụ: CEO, báo cáo tình hình fleet…" aria-label="Ra lệnh cho AI CEO" /><button disabled={!command.trim()}>Gửi ↗</button></form><div className={styles.quickRow}><span>Gợi ý</span>{quick.map(q => <button type="button" key={q} onClick={() => setCommand(q)}>{q}</button>)}<a href="/ceo" style={{ marginLeft: 8 }}>♛ Cockpit</a><a href="/website" style={{ marginLeft: 8 }}>Website</a></div></section>
      <section className={styles.footerGrid}><a href="/ceo" className={styles.linkCard}><span>♛</span><div><small>AI CEO</small><strong>Cockpit điều phối</strong><p>Fleet · cảnh báo · KPI · isolation.</p></div><b>→</b></a><a href="/agents" className={styles.linkCard}><span>◈</span><div><small>FLEET CONTROL</small><strong>20 AI Agents</strong><p>Trạng thái từng AI độc lập.</p></div><b>→</b></a><a href="/website" className={styles.linkCard}><span>⌁</span><div><small>AI WEBSITE</small><strong>Storefront live</strong><p>SalesBot · Tier A posts.</p></div><b>→</b></a></section>
    </main>

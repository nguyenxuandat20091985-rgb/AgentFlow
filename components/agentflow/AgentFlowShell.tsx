  ...
  return (
    <div className="af-shell">
      <aside className="af-sidebar">
        <a className="af-brand" href="/" aria-label="AgentFlow - Trang chủ">
          <img src="/avatar.svg" alt="" />
          <span>Agent<span>Flow</span></span>
        </a>
        <div className="af-side-caption">AI OPERATIONS</div>
        <nav className="af-side-nav" aria-label="Điều hướng">
          {nav.map(([href, icon, label]) => (
            <a key={href} className={(view === "overview" && href === "/") || href.includes(view) ? "active" : ""} href={href}>
              <span>{icon}</span><span>{label}</span>
            </a>
          ))}
        </nav>
        <div className="af-sidebar-footer">
          <span className="af-online-dot" />
          <div><strong>Control plane</strong><small>Supabase · PayOS · Live</small></div>
        </div>
      </aside>
    ...
    <div className="af-overview">
      <section className="af-panel af-control-center">
        <div className="af-section-head">
          <div>
            <div className="af-kicker">OPERATIONS</div>
            <h2>Trung tâm điều hành</h2>
            <p>Điểm vào duy nhất để điều phối AI CEO và theo dõi sức khỏe vận hành.</p>
          </div>
          <span className="af-live-pill"><i /> LIVE</span>
        </div>

        <div className="af-command-grid">
          <a href="/chat" className="af-command-card af-command-primary">
            <span className="af-command-icon">✦</span>
            <div><small>AI CEO</small><strong>Giao việc & điều phối</strong><p>Ra lệnh bằng ngôn ngữ tự nhiên, chọn đúng Agent và xem kết quả.</p></div>
            <b>→</b>
          </a>
          <a href="/agents" className="af-command-card">
            <span className="af-command-icon">◈</span>
            <div><small>FLEET</small><strong>Quản lý 20 AI Agents</strong><p>Kiểm tra trạng thái từng Agent và năng lực đang hoạt động.</p></div>
            <b>→</b>
          </a>
          <a href="/workflows" className="af-command-card">
            <span className="af-command-icon">⌁</span>
            <div><small>AUTOMATION</small><strong>Điều hành Workflows</strong><p>Theo dõi các quy trình tự động mà không pha trộn dữ liệu tài chính.</p></div>
            <b>→</b>
          </a>
        </div>

        <div className="af-control-footer">
          <div><span>AI đang hoạt động</span><strong>{loading ? "—" : `${running}/20`}</strong></div>
          <div><span>Giao dịch thành công</span><strong>{loading ? "—" : successfulPayments}</strong></div>
          <div><span>Nhịp đồng bộ</span><strong>10s</strong></div>
          <a href="/chat">Hỏi AI CEO →</a>
        </div>
      </section>
      ...
      <section className="af-panel af-system-status">
        <div className="af-section-head">
          <div>
            <div className="af-kicker">SYSTEM HEALTH</div>
            <h2>Trạng thái hệ thống</h2>
            <p>Biểu đồ vận hành và các dịch vụ nền, không lẫn với dòng tiền.</p>
          </div>
          <span className="af-health-score">{percent}% fleet online</span>
        </div>

        <div className="af-health-layout">
          <div className="af-donut-wrap">
            <div className="af-donut" style={{ background: `conic-gradient(#10B981 0 ${percent}%, #EF4444 ${percent}% 100%)` }} aria-label={`${percent}% AI đang chạy`}>
              <div><strong>{loading ? "—" : running}</strong><span>đang chạy</span></div>
            </div>
            <div className="af-legend"><span><i className="green" />Đang chạy <b>{running}</b></span><span><i className="red" />Đã dừng <b>{stopped}</b></span></div>
          </div>
          <div className="af-health-list">
            {health.map(([name, detail]) => (
              <div className="af-health-row" key={name}>
                <div className="af-health-icon">✓</div><div><strong>{name}</strong><span>{detail}</span></div><em>Healthy</em>
              </div>
            ))}
          </div>
        </div>
...
function Agents({ fleet, loading, money }: { fleet: Agent[]; loading: boolean; money: (value: number) => string }) {
  return <section className="af-page-section">
    <div className="af-section-head"><div><div className="af-kicker">AUTONOMOUS FLEET</div><h2>20 AI Agents</h2><p>Mỗi Agent có trạng thái và doanh thu riêng, tự cập nhật mỗi 10 giây.</p></div><span className="af-info-pill">20 / 20 Agents</span></div>
    <div className="af-agent-grid">
      {loading ? NAMES.map((name) => <div className="af-agent-skeleton" key={name} />) : fleet.map((agent, index) => {
        const on = agent.status === "running" || agent.status === "ready";
        return <article className={`af-agent-card ${on ? "is-on" : "is-off"}`} key={agent.id} tabIndex={0} aria-label={`${agent.name}: ${on ? "Đang chạy" : "Đã dừng"}`}>
          <div className="af-agent-top"><span className="af-agent-icon">{ICONS[index]}</span><span className={`af-status-dot ${on ? "on" : "off"}`} /></div>
          <strong>{agent.name}</strong><span className="af-agent-role">{agent.role || "AI Agent"}</span>
          <div className="af-agent-status">{on ? "Đang chạy" : "Đã dừng"}</div>
          <div className="af-agent-revenue">+{money(Number(agent.revenue || 0))}</div>
        </article>;
      })}
    </div>
  </section>;
...
function Workflows() {
  const items = [
    ["Customer Acquisition", "Lead → tư vấn → payment → webhook", "Active"],
    ["E-Commerce Autopilot", "Tìm deal → nội dung → theo dõi chuyển đổi", "Active"],
    ["Affiliate Autopilot", "Theo dõi click → đơn hàng → commission", "Active"],
    ["Revenue Reconciliation", "PayOS webhook → payment_events → revenue_ledger", "Protected"],
  ];
